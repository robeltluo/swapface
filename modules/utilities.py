...
def run_ffmpeg(args: List[str]) -> bool:
    ...
    print("run_ffmpeg ffmpeg command:", " ".join(commands))
    try:
...
def create_video(target_path: str, fps: float = 30.0) -> None:
    ...
    temp_directory_path = get_temp_directory_path(target_path)
    run_ffmpeg(
        ['-r', str(fps), '-i', os.path.join(temp_directory_path, '%04d.png'), '-c:v', modules.globals.video_encoder,
         '-crf', str(modules.globals.video_quality), '-pix_fmt', 'yuv420p', '-vf',
...
def rename_img_name(path: str, post_fix: str) -> str:
    dir_name = os.path.dirname(path)
    file_name = os.path.basename(path)
    targe_name, fix = os.path.splitext(file_name)
    return os.path.join(dir_name, targe_name + post_fix + fix)


def get_temp_directory_path(target_path: str) -> str:
    target_name, _ = os.path.splitext(os.path.basename(target_path))
    target_directory_path = os.path.dirname(target_path)
    print("return temp path:", os.path.join(target_directory_path, TEMP_DIRECTORY, target_name))
